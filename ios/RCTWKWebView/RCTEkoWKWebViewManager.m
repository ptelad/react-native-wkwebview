#import "RCTEkoWKWebViewManager.h"
...
@interface RCTEkoWKWebViewManager () <RCTWKWebViewDelegate>

@end

@implementation RCTEkoWKWebViewManager
{
...
- (UIView *)view
{
  RCTEkoWKWebView *webView = [[RCTEkoWKWebView alloc] initWithProcessPool:[[WKProcessPool alloc] init]];
...
RCT_EXPORT_VIEW_PROPERTY(onWKTerminated, RCTDirectEventBlock)
...
{
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RCTEkoWKWebView *> *viewRegistry) {
    RCTEkoWKWebView *view = viewRegistry[reactTag];
    if (![view isKindOfClass:[RCTEkoWKWebView class]]) {
      RCTLogError(@"Invalid view returned from registry, expecting RCTEkoWKWebView, got: %@", view);
    } else {
      [view goBack];
    }
  }];
...
- (BOOL)webView:(__unused RCTEkoWKWebView *)webView
